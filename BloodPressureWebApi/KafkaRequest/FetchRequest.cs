...
namespace BloodPressureWebApi.KafkaRequest
{
    ...
    public class FetchRequest : IRequestBuffer
    {
        ...
        public FetchTopicRequest AddTopic(String name)
        {
            var topic = new FetchTopicRequest(name);
            Topics.Add(topic);
            return topic;
        }

        public FetchTopicRequest AddTopic(String name, int partitionId, long fetchOffset, int maxBytes)
        {
