...
using Message = BloodPressureWebApi.Common.Message;
...
namespace BloodPressureWebApi.KafkaRequest
{
    ...
    public class ProduceRequest
    {
        ...
        public MessageSet AddMessage(string topicName, int partitionId, byte[] payload, int messageKey)
        {
            var kafkaMessageKey = Encoding.ASCII.GetBytes(messageKey.ToString());

            var topic = AddTopic(topicName);
            var partition = topic.AddPartition(partitionId);
            var messageSet = new MessageSet(payload);
            messageSet.Message.Key = kafkaMessageKey;
